...
cd "$(dirname "$0")" || {
  echo "[@flashcatcloud/mobile-react-navigation] WARNING: Failed to change directory."
}
...
safe_replace() {
  ...
  if [ -d "$dir" ]; then
    echo "[@flashcatcloud/mobile-react-navigation] Processing $dir ..."
    ...
    if [ $? -ne 0 ]; then
      echo "[@flashcatcloud/mobile-react-navigation] WARNING: Replacement failed for $dir"
      echo "[@flashcatcloud/mobile-react-navigation] Error details:"
      echo "$sed_errors"
      had_warnings=true
    elif [ -n "$sed_errors" ]; then
      # Sometimes sed emits warnings but exits successfully
      echo "[@flashcatcloud/mobile-react-navigation] NOTE: sed produced warnings for $dir:"
      echo "$sed_errors"
      had_warnings=true
    fi
  else
    echo "[@flashcatcloud/mobile-react-navigation] WARNING: Directory not found: $dir"
    had_warnings=true
  fi
}
...
if [ "$had_warnings" = true ]; then
  echo ""
  echo "[@flashcatcloud/mobile-react-navigation] ⚠️  IMPORTANT: Script completed with warnings."
  echo "[@flashcatcloud/mobile-react-navigation] React Navigation imports may not have been updated correctly."
  echo "[@flashcatcloud/mobile-react-navigation] Please review the logs above — your project’s navigation might not work as expected."
else
  echo "[@flashcatcloud/mobile-react-navigation] ✅ Completed successfully with no warnings."
fi
