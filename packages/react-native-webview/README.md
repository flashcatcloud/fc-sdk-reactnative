...
Import `WebView` from `@flashcatcloud/mobile-react-native-webview` instead of `react-native-webview`:
...
You can use all existing functionalities from `react-native-webview` as the `WebView` component from `@flashcatcloud/mobile-react-native-webview` wraps the `react-native-webview` component.
